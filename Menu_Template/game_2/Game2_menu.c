 ...
 */
static void render_home_menu(MapSystem* map) {

    LCD_Fill_Buffer(0);
    
    // Title
    LCD_printString("SELECT MAP", 50, 10, 1, 3);
    
    // Menu options with selection highlight
    for (int i = 0; i < NUM_MAP_OPTIONS; i++) {
        uint16_t y_pos = 70 + (i * 40);
        uint8_t text_size = 2;
        
        if (i == map->selected_option) {
        ...
        LCD_printString((char*)map_options[i], 70, y_pos, 1, text_size);
    }
...
MapState Map_Run(MapSystem* map) {
    ...
    while (1) {
        ...
        if (current_direction == S && last_direction != S) {  // Joystick pushed DOWN
            // Move selection down
            map->selected_option++;
            if (map->selected_option >= NUM_MAP_OPTIONS) {
                map->selected_option = 0;  // Wrap around
            }
        } 
        else if (current_direction == N && last_direction != N) {  // Joystick pushed UP
            // Move selection up
            if (map->selected_option == 0) {
                map->selected_option = NUM_MAP_OPTIONS - 1;  // Wrap around
            } else {
                map->selected_option--;
            }
        }
        ...
        if (current_input.btn3_pressed) {
            ...
            if (map->selected_option == 0) {
                selected_map = MENU_GAME2_MAP1;
            } else if (map->selected_option == 1) {
    ...
    return selected_map;  // Return which game was selected
}
